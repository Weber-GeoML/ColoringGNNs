...
Before running code from this repository, the Python libraries NetworkX, PyTorch, PyTorch Geometric, and NumPy must be installed.

## GNN-based Algorithms

The files `unmodified_gcn.py`, `mod_gcn.py`, and `full_gcn.py` each contain the algorithm suggested by their name. The functions are named `unmod_gcn_color`, `mod_gcn_color`, and `full_gcn_color` respectively.
